...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠸⢤⠼⠁⠀⠀⠳⣤⡼⠀⠀⠀⠀⠀⠀
  ...
  -- LSP Support
  {
    ...
    config = function()
      ...
      -- Install PHP debug adapter separately (not an LSP)
...
keymap("n", "<F5>",
  ":w<CR>:split | terminal g++ -g -std=c++17 -Wall -Wextra -o %:r % && ./%:r<CR>",
  { desc = "Compile and run C++" })

keymap("n", "<leader>cm",
  ":w<CR>:split | terminal g++ -g -fsanitize=address -std=c++17 -Wall -Wextra -o %:r % && ./%:r<CR>",
  { desc = "Compile with AddressSanitizer" })
...
vim.api.nvim_set_keymap("n", "<Leader>d0", ":lua require('neogen').generate({ type = 'class' })<CR>", opts)
